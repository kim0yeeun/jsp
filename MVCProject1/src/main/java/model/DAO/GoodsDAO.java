...
public class GoodsDAO  extends DataBaseInfo{
	final String COLUMNS = "  goods_num,  goods_name , goods_price, goods_date, goods_content ,"
												+ "  goods_images, goods_qty , goods_company,  emp_num  , "
												+ "  ip_addr "; 
	String sql;
	...
	public String getImages (String goodsNum) {
		...
		try {
			pstmt = con.prepareStatement(sql);
			// ? 하나 있으니까 값 전달 pstmt.set~
			pstmt.setString(1, goodsNum);
			// select 함냐ㅕ rs로 받아옴
			rs= pstmt.executeQuery();
			// rs받았스니가 if문
			if (rs.next()) {   // rs가 존재한다면 굿즈이미지 받아오기
				goodsImages = rs.getString(1);
			}
		} catch (Exception e) {e.printStackTrace();	}
		finally {close();}
		// 이미지를 없애야함 
		return goodsImages; // goodsDeletecontroller로 리컨됨 
	}
	
	public GoodsDTO  selectOne(String goodsNum) {
		GoodsDTO dto = new GoodsDTO();
		con = getConnection();
		sql = "  select " + COLUMNS + "  from goods  where goods_num = ? ";
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, goodsNum); // GoodsInfoController에서의 num 임 달라도 된다는걸 보여줌 
			rs = pstmt.executeQuery();
			if (rs.next()) {
